...
//about ten seconds
console.log('test weather time to Cache' , testTimeToCache);
...
Weather.weatherRequest = async (request, response) => {
  ...
  try {
    ...
    console.log('did we find city name for weather?', cityName);
    ...
    if(cache[key] &&( Date.now() - cache[key].timeStamp) < acceptableTimeToCache){
      ...
      console.log([key], ' weather is in the cache already');
      ...
      if (needApiWeather === undefined) {
        console.log('needApi weather undefined?')
